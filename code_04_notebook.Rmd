...
get_rsi_and_slice_into_windows <- function( slices = 24, by = by_stock){
  ...
  by_stock_window <- by_stock_window %>%
  ...
  return(by_stock_window)
  
}

by_stock_window <- get_rsi_and_slice_into_windows()
...
get_grid <- function(){
buy  <- seq(20, 70, length.out = 6) %>%  round(digits = 2)

sell <- seq(71, 90, length.out = 4) %>%  round(digits = 2)

buy_sell_percent <- .1 # seq(.1, .5, length.out = 4) %>% round(digits = 2)

args <- expand.grid(buy = buy, 
                    sell = sell,
...
by_stock_window1 <- by_stock_window1 %>%
