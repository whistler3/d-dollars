...
sim_results <- run_simulation(by_window = by_window_stock, buy = 29, sell = 50, percent = .1, cash_on_hand = 5000)
sim_results[1]  
sim_results[2]
  # 
...
```
```{r}
ds <- sim_results[1] %>% as.data.frame %>% ungroup 

ds<- ds %>% group_by(stock) %>% arrange(date)
ds
```
